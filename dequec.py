...
#################################################
# Deque ADT implementation, using circular list #
#################################################

class DequeC(Generic[T]):
  ...
  # Public methods that are essentially same as Queue
  ...
  def back(self) -> T:
    return self._data.read(self._end)

  def dequeue_back(self) -> T:
    back = self.back()

    self._data.write(self._end, None)   # 1
    self._n -= 1        # 2
    # No change to self._start    # 3
    if self._end == 0:
      self._end = self._capacity - 1
    else:
      self._end -= 1           # 4
    # If data list is more than 75% unoccupied, then reduce its size
    if self._n < self._capacity * 0.25:
  ...
  def enqueue_front(self, value: T) -> None:
    ...
    self._data.write(self._start, value)   # 1
    self._n += 1             # 2
    # self._start already changed  # 3
    # self._end no change   # 4
    # self._capacity changed if needed   # 5
